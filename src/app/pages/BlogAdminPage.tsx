...
import { BLOG_ADMIN_STORAGE_KEY, blogSiteQuery } from "@/app/config/blog";

/** sessionStorage key for the bearer token typed on /blog-admin (must match Edge secret BLOG_ADMIN_SECRET). */
const STORAGE_KEY = BLOG_ADMIN_STORAGE_KEY;
...
export function BlogAdminPage() {
  ...
  const loadPosts = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(supabaseEdgeUrl(`/admin/blog/posts${blogSiteQuery()}`), { headers: authHeaders() });
  ...
  const deletePost = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(supabaseEdgeUrl(`/admin/blog/posts/${id}${blogSiteQuery()}`), {
        method: "DELETE",
        headers: authHeaders(),
      });
      const json = await res.json().catch(() => ({}));
    ...
    return (
      <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center p-6">
        <div className="w-full max-w-md space-y-6">
          <h1 className="text-2xl font-semibold">Blog admin (CBHC)</h1>
          <p className="text-slate-300 text-sm leading-relaxed">
            Enter the same secret you configured as <code className="text-brand-teal-light">BLOG_ADMIN_SECRET</code> in Supabase
          ...
          <form onSubmit={handleSaveSecret} className="space-y-4">
            ...
            />
            <Button type="submit" className="w-full bg-brand-teal hover:bg-brand-teal-hover">
              Continue
            </Button>
          </form>
        </div>
              ...
              {posts.map((p) => (
                <li
                  ...
                  className={`rounded-lg border p-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 ${
                    editingId === p.id ? "border-brand-teal bg-brand-teal/10" : "border-slate-200 bg-white"
                  }`}
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                ...
                <div className="space-y-2 min-w-0">
                  ...
                  <div
                    className={cn(
                      ...
                      bodyDragOver && "ring-2 ring-brand-teal ring-offset-2 ring-offset-white",
                    )}
                  >
                    <Textarea
                      ref={bodyRef}
...
\`\`\``}
            ...
            <Button type="submit" disabled={saving} className="bg-brand-teal hover:bg-brand-teal-hover">
              {saving ? (
                <>
