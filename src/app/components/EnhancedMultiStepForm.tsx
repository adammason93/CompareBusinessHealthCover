...
import { 
  Home, Users, Briefcase, UserCircle, HelpCircle, Heart, User,
...
export function EnhancedMultiStepForm({ onSubmit, onBack, user, onOpenAuth }: MultiStepFormProps) {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors, touchedFields } } = useForm<FormData>({
    mode: "onChange",
    defaultValues: typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('healthFormDraft') || '{}') : {}
  });
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (typeof window !== 'undefined' && currentStep > 1) {
        localStorage.setItem('healthFormDraft', JSON.stringify(watchedFields));
        setSavedDraft(true);
              ...
              {currentStep === 1 && (
                <div className="space-y-4 sm:space-y-6">
                  <div>
                    ...
                    <p className="text-sm sm:text-base text-gray-600">Select the type of insurance you're interested in</p>
                  </div>
                  ...
                  <div className="space-y-3">
                    <OptionButton
                      ...
                      value="family-health"
                      description="Private medical bills coverage for your family"
                      currentValue={watchedFields.compareType || ''}
                      onClick={() => setValue("compareType", "family-health")}
                      icon={Users}
                    />
                    <OptionButton
                      id="B"
                      label="Individual Health Insurance"
                      value="individual-health"
                      description="Private medical bills coverage for yourself"
                      currentValue={watchedFields.compareType || ''}
                      onClick={() => setValue("compareType", "individual-health")}
                      icon={User}
                    />
                    <OptionButton
                      id="C"
                      ...
                      value="sme-health"
                      description="Health cover for small and medium enterprises"
                      currentValue={watchedFields.compareType || ''}
                    ...
                    <OptionButton
                      ...
                      description="Health cover for large organizations"
                      currentValue={watchedFields.compareType || ''}
