        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="mt-1 text-gray-600 text-sm sm:text-base">
              Tips and guides on business health insurance for UK SMEs.
            </p>
          </div>
          <button
            type="button"
            onClick={() => onNavigate("blog")}
            className="inline-flex items-center gap-2 text-brand-navy font-medium text-sm hover:text-brand-teal self-start sm:self-auto"
          >
            View all posts
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {posts.map((p) => (
              <article
                key={p.id}
                className="flex flex-col bg-gray-50 rounded-xl border border-gray-200 overflow-hidden hover:border-brand-teal/40 hover:shadow-md transition-shadow"
              >
                <button
                  type="button"
                  onClick={() => onNavigate(`blog/${p.slug}`)}
                  className="text-left flex flex-col flex-1 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-teal rounded-xl"
                >
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    <span className="mt-3 text-sm text-brand-navy font-medium">Read more</span>
                  </div>
                </button>
              </article>
            ))}
